...
- [x] M1-08 – Persist the photo filename that produced the best OCR result into `draft.metadata` and store it inside the DB for debugging.
...
- [x] M2-05 – Display the FastAPI version returned by `/health` and warn if it’s outdated vs bundled schema version. (2025-11-19 – Connect screen now shows server version vs expected schema and highlights mismatches.)
...
- [x] M2-08 – Implement pagination/infinite scroll for `/api/drafts` results using the backend’s new limit/offset params. (2025-11-19 – Draft list now requests pages via limit/offset and auto-loads more on scroll.)
- [x] M2-09 – Create skeleton shimmer placeholders while loading drafts to avoid blank jumps. (2025-11-19 – Draft list now shows animated skeleton cards during initial load.)
- [x] M2-10 – Add a Draft Filters sheet (status, brand, size) and pass the selected filters to the API query string. (2025-11-19 – Draft list now exposes a filters sheet feeding status/brand/size params to `/api/drafts`.)
