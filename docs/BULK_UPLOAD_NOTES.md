...
- Flow: pick many photos (up to MAX_BULK_PHOTOS). Photos are sorted by `creationTime` and grouped; a new draft starts when the gap between photos exceeds `BULK_TIME_GAP_SECONDS` (default 90s). Oversized groups are split into chunks of `MAX_PHOTOS_PER_DRAFT` (default 8).
